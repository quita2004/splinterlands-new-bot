...
const splinterlandsPage = require('./src/splinterlandsPage');
const user = require('./src/user');
const card = require('./src/cards');
const { clickOnElement, getElementText, getElementTextByXpath, teamActualSplinterToPlay } = require('./src/helper');
const quests = require('./src/quests');
const ask = require('./src/possibleTeams');
const chalk = require('chalk');

const limitERC = process.env.LIMITERC ? parseInt(process.env.LIMITERC) : 75;

// LOAD MY CARDS
async function getCards(username) {
    const myCards = await user.getPlayerCards(username.split('@')[0]) //split to prevent email use
    return myCards;
}

async function getQuest(username) {
    return quests.getPlayerQuest(username.split('@')[0])
        .then(x => x)
...
async function callLogin(page, username, pass){
    ...
    if (item != undefined) {
        ...
        await splinterlandsPage.login(page, username, pass).catch(e => {
            console.log(e);
...
async function startBotPlayMatch(page, myCards, quest, username, pass) {
    console.log(new Date().toLocaleString())
    if (myCards) {
        console.log(username, ' count total card: ' + myCards.length)
    } else {
        console.log(username, ' playing only basic cards')
    }
    await callLogin(page, username, pass);
    ...
    if (process.env.CLAIM_SEASON_REWARD === 'true') {
        try {
            ...
            await page.waitForSelector('#claim-btn', { visible: true, timeout: 3000 })
                .then(async (button) => {
                    button.click();
                    console.log(`claiming the season reward. you can check them here https://peakmonsters.com/@${username}/explorer`);
                    await page.waitForTimeout(20000);
                    await page.reload();

                })
                .catch(() => console.log(`no season reward to be claimed, but you can still check your data here https://peakmonsters.com/@${username}/explorer`));
            await page.waitForTimeout(3000);
            await page.reload();
        }
        catch (e) {
    ...
    // lấy những thẻ ưu tiên, kiểm tra xem mình có những thẻ này ko
    ...
    const possibleTeams = await ask.possibleTeams(matchDetails, priorityCards).catch(e => console.log('Error from possible team API call: ', e));
    ...
    try {
        ...
        try {
            ...
            if (winner.trim() == username.split('@')[0]) {
...
async function Run(username, pass) {
    console.log('START ', username, new Date().toLocaleString())
    ...
    const myCards = await getCards(username)
    ...
    const quest = await getQuest(username);
    if (!quest) {
    ...
    while (true) {
        try {
            let result = await startBotPlayMatch(page, myCards, quest, username, pass)

            if (result === 1) {
                console.log(`Stop by Limit ERC: ${username} ${limitERC}`)
                break
            }
        } catch (e) {
            console.log('Routine error at: ', new Date().toLocaleString(), e)
        }
        await console.log(username, 'waiting for the next battle in', sleepingTime / 1000 / 60, ' minutes at ', new Date(Date.now() + sleepingTime).toLocaleString())
...
(async () => {
    let content = fs.readFileSync('./data.txt', { encoding: 'utf8' });

    content = content.trim();
    let userList = content.split('\n');

    userList.forEach(async userString => {
        let [username, pass] = userString.split(' ')

        if (username && pass) {
            console.log(`Start run ${username}`);
